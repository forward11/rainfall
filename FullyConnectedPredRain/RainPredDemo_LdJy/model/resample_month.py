...
def delete_month(filename):
    ...
    df =df.drop(df[df['time']=='2019-12-31'].index)
...
def add_lps(filename):
    ...
    with open(os.path.join(os.path.dirname(__file__),'config.json'),'r') as json_file:
            config = json.load(json_file)
    df = pd.read_csv(os.path.join(config['directory'],'{}_resample.csv').format(filename))
    df_old = pd.read_csv(os.path.join(config['directory'],'{}_old.csv').format(filename))
    df_old['RPH_jy'] = df['RPH']
    # df_old.drop(['Unnamed: 0'], axis=1, inplace=True)
    df_old.to_csv((os.path.join(config['directory'], '{}_resample.csv').format(filename)),index=False)
